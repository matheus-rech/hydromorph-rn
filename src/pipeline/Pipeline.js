...
export async function runMultiModelPipeline(volume, onProgress = () => {}) {
  // Run the classical pipeline (steps 0-8)
  const classicalResults = await runPipeline(volume, onProgress);
  ...
  const allResults = {
    classical: {
      ...classicalResults,
      modelId: 'classical',
      modelName: 'Classical (Proprietary)',
      modelColor: getModelConfig('classical').color,
      colorRgb: getModelConfig('classical').colorRgb,
      boundingBoxes: [],
      processingTime: '—',
      processingTimeNum: 0,
    },
